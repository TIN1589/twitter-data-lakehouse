...
def generate_mock_tweets(count=20):
    """Generate a list of mock tweet dictionaries from real Twitter dataset.
    ...
    """
    dataset_path = os.path.join(os.path.dirname(__file__), "real_tweets_dataset.json")
    
    # Fallback to hardcoded mock generation if file is missing
    ...
    # Sample count number of tweets
    sampled_tweets = random.sample(real_tweets, min(count, len(real_tweets)))
    
    base_time = datetime.utcnow()
    tweets = []

    for item in sampled_tweets:
        # Generate a dynamic timestamp within the last 24 hours
        created_at = base_time - timedelta(
            minutes=random.randint(1, 1440),
            seconds=random.randint(0, 59),
        )
        
        # Slightly fluctuate public metrics to simulate dynamic user interaction
        fluctuation_factor = random.uniform(0.9, 1.2)
        metrics = item.get("public_metrics", {})
        
        tweet = {
            "id": item.get("id", str(random.randint(10**17, 10**18 - 1))),
            "text": item.get("text", ""),
            "created_at": created_at.strftime("%Y-%m-%dT%H:%M:%S.000Z"),
            "author_id": item.get("author_id", "44196397"),
            "username": item.get("username", "elonmusk"),
            "lang": item.get("lang", "en"),
            "public_metrics": {
                "like_count": int(metrics.get("like_count", 1000) * fluctuation_factor),
                "retweet_count": int(metrics.get("retweet_count", 200) * fluctuation_factor),
                "reply_count": int(metrics.get("reply_count", 50) * fluctuation_factor),
                "quote_count": int(metrics.get("quote_count", 20) * fluctuation_factor),
            },
